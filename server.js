// server.js
// --- VERSION 24.0 (Final Version with Gender Filtering) ---
// This version intelligently detects gender keywords in the search query
// and applies the appropriate category filter for more accurate results.
...
const translationMap = {
    ...
    // Demographics (will be removed from search query)
    'גברים': 'men', 'לגבר': 'men',
...
const genderKeywords = {
    men: ['men', 'man', 'boy', 'גברים', 'לגבר', 'גבר', 'ילד'],
    women: ['women', 'woman', 'girl', 'נשים', 'לאישה', 'אישה', 'ילדה']
};

const genderCategoryIds = {
    men: '200001655', // Example ID for Men's Clothing
    women: '200000345' // Example ID for Women's Clothing
};
...
app.get('/search', async (req, res) => {
    ...
    // --- NEW: Gender Detection and Category Assignment ---
    const lowerCaseKeywords = keywords.toLowerCase();
    let detectedGender = null;

    for (const gender in genderKeywords) {
        for (const keyword of genderKeywords[gender]) {
            if (lowerCaseKeywords.includes(keyword)) {
                detectedGender = gender;
                categoryId = genderCategoryIds[gender];
                // Remove the gender keyword from the search query for cleaner search
                keywords = keywords.replace(new RegExp(keyword, 'gi'), '').trim();
                console.log(`[DEBUG] Detected gender: ${gender}, setting categoryId to ${categoryId}`);
                break;
            }
        }
    ...
    const performSearch = async () => {
        ...
        // Add category_id if a gender was detected
        if (categoryId) {
