// server.js
// --- VERSION 7.4 (Deployment Version with Multi-Word Translation) ---
// This version improves the translation logic to handle multi-word queries
// by translating each recognized word.
...
app.get('/search', async (req, res) => {
    ...
    // --- EVEN MORE ROBUST Translation Logic ---
    if (isHebrew(keywords)) {
        console.log(`Detected Hebrew query: "${keywords}"`);
        const words = keywords.split(' ');
        const translatedWords = [];

        // Translate each word if it exists in the map
        words.forEach(word => {
            const lowerCaseWord = word.toLowerCase();
            if (translationMap[lowerCaseWord]) {
                translatedWords.push(translationMap[lowerCaseWord]);
            }
        });
        ...
        if (translatedWords.length > 0) {
            ...
            console.log(`No translatable keywords found in "${keywords}", using original query.`);
        }
    }
