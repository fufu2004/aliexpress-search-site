// server.js
// --- VERSION 26.0 (Final Version with AI-Powered Translation) ---
// This version uses a live AI model for translation, reading the Google API Key
...
const GOOGLE_API_KEY = process.env.GOOGLE_API_KEY; // --- NEW: Google API Key for Translation ---
...
async function translateHebrewToEnglish(text) {
    ...
    const geminiApiKey = GOOGLE_API_KEY; // Use the key from environment variables
...
app.get('/search', async (req, res) => {
    ...
    if (!APP_KEY || !APP_SECRET || !ACCESS_TOKEN || !REFRESH_TOKEN || !TRACKING_ID || !GOOGLE_API_KEY) {
        return res.status(500).json({ error: 'Server is not configured correctly. Please check all environment variables on Render, including GOOGLE_API_KEY.' });
    }
